...
const CardFunctions = ({isDragging}) => {
  ...
  return (
    <div className='flex flex-row justify-between items-center space-x-10 px-6 py-2'>
        ...
        <button onClick={() => isDragging && showCardDetails(isDragging)} className='flex flex-row text-[color:var(--secondary-text--color)] hover:text-[color:var(--primary-text--color)] transition ease-in-out delay-150 p-2 border-2'>{messageIcon}</button>
        <button onClick={deleteCard} className='flex flex-row text-[color:var(--secondary-text--color)] hover:text-[color:var(--primary-text--color)] transition ease-in-out delay-150 p-2 border-2'>{deleteIcon}</button>
        <button onClick={updateDeadline} className='flex flex-row space-x-2 text-[color:var(--secondary-text--color)] hover:text-[color:var(--primary-text--color)] transition ease-in-out delay-150 p-2 border-2'>{calendarIcon}</button>
    </div>
  )
}
