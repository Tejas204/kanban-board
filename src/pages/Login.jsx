  ...
  const handleLogin = async (event) => {
    ...
    try {
      const { data } = await axios.post(
        console.log(email, password)`${server}/users/login`,
        {
          email,
          password,
        },
      ...
      <div className="flex h-[100%] w-[100%] justify-center items-center">
        <div className="flex flex-col rounded-md mt-[3%] py-10 px-16 gap-y-14 shadow-xl text-[color:var(--primary-text--color)] bg-[color:var(--card-bg--color)]">
          ...
          <form className="flex flex-col gap-y-5" onSubmit={handleLogin}>
            ...
            <Link
              to="/resetPassword"
              className="text-center text-lg text-[color:var(--primary-text--color)] underline decoration-2 underline-offset-8 hover:decoration-[color:var(--button-bg--color)] transition ease-linear duration-150"
            >
              Forgot password?
            </Link>
          </form>
